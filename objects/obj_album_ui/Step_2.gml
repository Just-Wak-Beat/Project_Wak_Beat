...
if (global.select_difficulty = 0 && global.show_new_songs <= 0)
{
	...
	else if (mouse_check_button_released(mb_left) && point_distance(mouse_x,mouse_y,global.c_x+104,global.c_y+920) < 80) || keyboard_check_pressed(vk_tab)
	{
		...
		else
		{
			...
			global.unlocked_music_name_new_list[1] = "(입력이 완료되었다면 화면을 꾹 홀드하거나, Enter키를 홀드하여 주세요)";
			keyboard_string = ""
			global.unlocked_music_name_new_list_color[1] = merge_color(c_black,c_white,0.7);
			global.unlocked_music_name_new_list[2] = "(입력이 완료되었다면 화면을 꾹 홀드하거나, Enter키를 홀드하여 주세요)";
