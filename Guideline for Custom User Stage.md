...
커스텀 맵 파일들은 게임 실행파일이 있는 위치에 ```커스텀 맵 파일```폴더 속에 존재합니다.

커스텀 맵을 생성할 경우, ```custom_map_file_(생성 날짜)```<b> 숫자가 적혀진 폴더 </b>가 생성됩니다.

*커스텀 맵 폴더 이름은 원하는 이름으로 마음대로 변경 가능합니다. <b> (게임을 종료한 상태여야 변경 가능합니다) </b>
...
<details>
<summary>제작한 커스텀 맵 공유 <b> [클릭] </b></summary>
...
공유하고 싶은 커스텀 맵 파일(폴더)를 ```우클릭``` 하여 ```압축하기```를 해줍니다.

이후 생성된 압축파일을 공유해주시면 됩니다.
...
|왁물원에 업로드|
|---|
|왁물원에 업로드 된 파일은 유저들이 직접 다운받아, 적용하여 플레이 가능합니다.|


|개발자에게 맵 파일 보내기|
|---|
|<div align = "center">가이드 라인 맨 아래에 있는 디스코드 링크를 통해</br>```Project Wak Beat 디스코드 서버```에 접속하여 DM으로 맵 파일을 보내주시면,</br>직접 검수 후 ```공식 오리지널 맵``` 혹은 ```공식 커스텀 맵```으로 채택되어</br>게임을 다운받는 모든 플레이어들에게 해당 맵 파일이 자동으로 적용됩니다.</div>|
